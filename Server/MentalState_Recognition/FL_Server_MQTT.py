...
class FederatedLearningServer:
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            print(f"Server connected to MQTT broker at {MQTT_BROKER}:{MQTT_PORT}")
            self.mqtt_client.subscribe(TOPIC_CLIENT_REGISTER)
            self.mqtt_client.subscribe("fl/client/+/update")
    ...
    def distribute_initial_model(self):
        ...
        self.mqtt_client.publish(TOPIC_TRAINING_CONFIG, json.dumps(self.training_config))
        ...
        self.mqtt_client.publish(TOPIC_START_TRAINING, json.dumps({"round": self.current_round}))
    ...
    def finish_training(self):
        ...
        time.sleep(2)  # Give clients time to receive message
    ...
    def save_results(self):
        ...
        results_file = os.path.join(results_dir, f"mqtt_{network_scenario}_training_results.json")
    ...
    def run(self):
        ...
        print("\n" + "=" * 70)
        print("Federated Learning Server - Mental State Recognition (MQTT)")
        print("=" * 70)
