...
class ServerQuantizationHandler:
    ...
    def __init__(self, config: Optional[QuantizationConfig] = None):
        ...
        self.pruner = None
        self.pruning_step = 0
        # Float32 FedAvg result from the last aggregate_compressed_updates (dequantize → avg → requantize path)
        self.last_aggregated_float_weights: Optional[List[np.ndarray]] = None
    ...
    def aggregate_compressed_updates(
        ...
        """
        FedAvg on quantized client payloads: dequantize → weighted average in float32 → re-quantize.

        Clients send ``Quantization.compress()`` dictionaries; the server decompresses each
        update, averages in full precision, then compresses the global model again for the downlink.
        ...
        Returns:
          (recompressed_global_dict, stats). Also sets ``last_aggregated_float_weights`` to the
          float FedAvg result for server-side bookkeeping.
        """
        print(f"\n{'='*70}")
        print(f"Aggregating quantized client updates (dequantize → FedAvg → requantize)")
        print(f"{'='*70}")

        if not client_updates:
            raise ValueError("No client updates provided")

        aggregated_weights, agg_stats = self.aggregate_quantized_updates(
            client_updates, aggregation_method=aggregation_method
        )
        self.last_aggregated_float_weights = aggregated_weights

        recompressed = self.compress_global_model(aggregated_weights)

        stats = {
            **agg_stats,
            "path": "dequantize_fedavg_requantize",
            "kept_quantized": False,
        }

        print(f"✓ Re-quantized global model for downlink: {len(recompressed.get('compressed_data', []))} layers")
        print(f"{'='*70}\n")

        return recompressed, stats
