"""FL termination flags — shared by all MentalState client entrypoints (Docker + native)."""
import os
...
def stop_on_client_convergence() -> bool:
    ...
    if use_ql:
        print("[Client] Auto-detected USE_QL_CONVERGENCE=True → using fixed_rounds mode (no early stopping)")
        return False
