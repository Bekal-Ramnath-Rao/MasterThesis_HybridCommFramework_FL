#!/usr/bin/env python3
"""
Build one uplink Q-table (3 × 12 × 5 in flat view per scenario) from archived training pickles.

Your on-disk ``q_table_emotion_uplink_trained.pkl`` is often all zeros after a fresh reset; archives
``shared_data/q_table_emotion_uplink_trained_archive_*.pkl`` retain partial Q-learning updates.
This script picks the archive with the strongest updates on each coarse scenario slice and merges
them into a single (3, 3, 2, 2, 5) array, then writes Markdown + CSV + a loadable pickle.

Default merge (emotion client, paths under repo ``shared_data/``):
  - excellent slice ← archive ...182029_good.pkl  - moderate slice  ← archive ...192556_moderate.pkl
  - poor slice      ← archive ...234520_congested_light.pkl
"""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Directory containing q_table_emotion_uplink_trained_archive_*.pkl",
    )
    ap.add_argument(
        "--out-dir",
    ...
    args = ap.parse_args()
    pattern = os.path.join(args.shared_data, "q_table_emotion_uplink_trained_archive_*.pkl")
    ...
    out_md = os.path.join(out_dir, "q_table_emotion_uplink_MERGED_from_training.md")
    out_pkl = os.path.join(out_dir, "q_table_emotion_uplink_merged_from_training_archives.pkl")
    csv_prefix = os.path.join(out_dir, "q_table_emotion_uplink_merged")
