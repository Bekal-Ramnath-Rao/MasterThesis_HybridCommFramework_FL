...
echo ""
"$PYTHON" "$SCRIPT_DIR/test_fl_network_system.py"
echo ""
...
echo ""
"$PYTHON" "$SCRIPT_DIR/fl_network_monitor.py" \
    --client-id 1 \
    --latency 200ms \
    --bandwidth 1mbit \
    --loss 2 \
...
echo ""
"$PYTHON" "$SCRIPT_DIR/fl_network_monitor.py" --client-id 1 --latency 300ms
echo ""
"$PYTHON" "$SCRIPT_DIR/fl_network_monitor.py" --show-status
echo ""
read -p "Press Enter to continue..."
...
echo ""
echo "Next steps:"
echo "  • Start monitoring: ${PYTHON} fl_training_dashboard.py"
echo "  • Interactive mode: ${PYTHON} fl_network_monitor.py --monitor"
